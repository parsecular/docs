...
Send an auth message immediately after connecting using either an API key or a terminal token:
...
`exchange_ts_ms` and `ingest_ts_ms` are included when available and provide epoch-millisecond timestamps for the exchange event time and server ingest time respectively.
...
Activity messages may also include optional `exchange_ts_ms` and `ingest_ts_ms` fields when available. Fill messages may also include `liquidity_role`: `"maker"`, `"taker"`, or omitted when unavailable. Present on `kind: "fill"` messages only.
...
| 1001 | `already_authenticated` | Already authenticated (duplicate auth message) |
...
| 3002 | `max_subscriptions` | Max subscriptions reached (includes tier and limit in message) |
...
|-------|------|-----|-------|
| Max subscriptions (total across connections) | 5 | 500 | 2000 |
| Max orderbook depth (levels/side) | 10 | 100 | 100 |
| Subscribe rate limit | 3 req/sec | 3 req/sec | 3 req/sec |
| Authentication timeout | 15 sec | 15 sec | 15 sec |

When you exceed the subscription limit, the server responds with error code `3002`. The error message includes your current tier and limit:

```json
{ "type": "error", "code": 3002, "message": "max subscriptions reached (limit: 5, tier: free)" }
```
  ...
  switch (msg.type) {
    case 'auth_ok':
      ...
      ws.send(JSON.stringify({
        type: 'subscribe',
        markets: [{ parsec_id: 'polymarket:572473', outcome: 'yes', depth: 10 }]
      }));
      break;
