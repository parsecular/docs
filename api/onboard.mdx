...
<ResponseField name="status" type="string">
  `complete` on success, or `already_linked` when managed Polymarket onboarding
  is re-run for an account that is already linked.
</ResponseField>
...
<ResponseField name="steps_completed" type="string[]">
  ...
  When `status` is `already_linked`, this list is empty.
</ResponseField>
