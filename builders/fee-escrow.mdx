...
const config = await client.builder.escrow.config();
console.log(config.fee_escrow_enabled, config.fee_bps);
```
```python Python
config = client.builder.escrow.config()
print(config.fee_escrow_enabled, config.fee_bps)
```
</CodeGroup>
