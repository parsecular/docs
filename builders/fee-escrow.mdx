...
Fee escrow is configured by Parsec admins during your onboarding call. The admin
will set:

- **`fee_escrow_enabled`** — whether fee escrow is active for your builder account
- **`fee_bps`** — the fee rate in basis points (e.g. 25 = 0.25%)
- **`affiliate_address`** — your 0x wallet address that receives fee payouts

<Note>
You cannot modify these settings yourself. To change your fee escrow config,
contact your Parsec admin on [Discord](https://discord.gg/754dYDXRrx).
</Note>
...
|-------|------|-------------|
| `fee_escrow_enabled` | boolean | Whether fee escrow is active |
| `fee_bps` | number | Fee in basis points (1-10000). 25 = 0.25% |
| `affiliate_address` | string | 0x-prefixed Ethereum address receiving fee payouts |
| `escrow_contract_address` | string | The escrow smart contract address |
| `min_fee_usdc` | string | Minimum fee in USDC base units |
| `min_fee_bps` | number | Minimum fee in basis points |
| `treasury_address` | string | Parsec treasury address |
| `affiliate_fee_bps` | number | Affiliate's share of fees in basis points (e.g. 2000 = 80% to you, 20% to treasury) |
...
When the order fills on the exchange:

| Recipient | Share | Example (0.25% fee on $55 order) |
|-----------|-------|----------------------------------|
| **Your affiliate address** | `affiliate_fee_bps` of the proportional fee | 80% of $0.1375 = $0.11 |
| **Parsec treasury** | Remainder | 20% of $0.1375 = $0.0275 |

The split is based on `affiliate_fee_bps` configured in the escrow contract
(default: 2000 = 20% to treasury, 80% to affiliate).
