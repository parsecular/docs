...
| `customer_id` | `POST /builder/users` response | Impersonate via `X-Builder-Customer-Id` |
| `external_id` | You choose this | Your internal ID (idempotency key) |
| `api_key` | `POST /builder/users` response | Not needed for impersonation (optional backup) |
...
## Per-request credentials (Mode B)

For advanced use cases, you can pass exchange credentials per-request instead
of using stored credentials. This avoids storing sensitive keys on Parsec.

Add the `X-Exchange-Credentials` header with base64-encoded JSON:
...
Builder requests draw from your **QPS pool** (default 500 QPS). Each end-user
has a configurable `qps_limit` that caps their individual usage within the pool.

Check your pool allocation:
...
console.log(pool.qps_pool, pool.total_allocated_qps, pool.end_user_count);
```
```python Python
resp = requests.get(
...
Builders support two wallet types for end-users. Both work with impersonation,
order routing, and fee escrow. See
[User Management](/builders/user-management#wallet-setup) for setup details.

| Wallet type | Setup | When to use |
|-------------|-------|-------------|
| **Managed (Privy EOA)** | `POST /builder/users/:id/wallet` | User doesn't have a wallet — Parsec creates one |
| **External** | `wallet_address` at user creation | User already has MetaMask or another EVM wallet |
