...
- `403 Forbidden`: credentials/authorization issue (fix credentials or permissions before retrying).
- `429 Too Many Requests`: rate limit exceeded. Includes a `Retry-After` header indicating seconds to wait. The SDKs retry automatically with exponential backoff.
- `503 Service Unavailable`: upstream exchange/network issue (retry with backoff).
...
<CardGroup cols={2}>
  ...
  <Card title="API Reference" icon="code" href="/api/list-markets">
